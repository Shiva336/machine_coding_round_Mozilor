...
Every public method receives a database ``connection`` as its first
...
class AuthService:
    ...
    async def _generate_and_store_tokens(
        ...
        await self.dao.insert_refresh_token(conn, user_id, jti, expires_at)
        return {
    ...
    async def register_user(
        ...
        try:
            user = await self.dao.insert_user(conn, email, hashed)
        except asyncpg.UniqueViolationError:
            logger.warning(
                "Registration attempt for already-registered email: %s", email
            )
            raise HTTPException(
        ...
        tokens = await self._generate_and_store_tokens(conn, user["id"])
    ...
    async def login_user(
        ...
        """
        user = await self.dao.find_user_by_email(conn, email)
    ...
    async def refresh_access_token(
        ...
        token_row = await self.dao.find_refresh_token(conn, jti)
        if token_row is None:
            ...
            await self.dao.revoke_all_user_refresh_tokens(conn, user_id)
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        if token_row["expires_at"].replace(tzinfo=timezone.utc) < datetime.now(
            ...
            logger.warning(
                "Expired refresh token used: user_id=%d jti=%s", user_id, jti
            )
            await self.dao.revoke_refresh_token(conn, jti)
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        return await self._generate_and_store_tokens(conn, user_id)
